    ...
    if ref_audio is not None:
        ...
        for spk_id, character_name in enumerate(ref_audio.split(",")):
            if not character_name.startswith("profile:"):
                prompt_audio_path = os.path.join(f"{CURR_DIR}/speakers/wav", f"{character_name}.wav")
                prompt_text_path = os.path.join(f"{CURR_DIR}/speakers/txt", f"{character_name}.txt")
...
def main(
    ...
    while True:
        ...
        sid = input("Enter speaker id: ").strip()
        n = int(input("Enter number of samples to generate: ").strip())
        out_prefix = input("Enter output file prefix: ").strip()

        for i in range(n):
            out_path = f"{CURR_DIR}/generated_outputs/{out_prefix}-{sid}-{i}.wav"

            if sid.startswith("f"):
                scene_prompt = f"{CURR_DIR}/scene_prompts/female_speaker.txt"
            elif sid.startswith("m"):
                scene_prompt = f"{CURR_DIR}/scene_prompts/male_speaker.txt"
            else:
                scene_prompt = None
            
            if scene_prompt is not None and os.path.exists(scene_prompt):
            ...
            transcript = transcript.replace("°C", "도씨")
